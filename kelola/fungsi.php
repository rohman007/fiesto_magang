...
function dasboarditem($paket) 
{
    ...
    switch ($paket) {
        case 1: //toko online
            ...
            list($neworder,$waitsendorder,$confirmorder) = $mysql->fetch_row($q);
...
function aksipagination2($namamodul, $screen = '', $prmtr = '', $search='', $title = array(), $type = '') {
    global $pages, $cfg_app_url, $urlfunc, $start;
	global $cfg_per_page;
	...
	if ($page > 0) {
		if ($prmtr == "") {
			$hal .= "<a href=\"" . $urlfunc->makePretty("?p=$namamodul$tambahan&screen=1", $title) . "$product_type\">$_first</a>\r\n";
			$hal .= "<a href=\"" . $urlfunc->makePretty("?p=$namamodul$tambahan&screen=$prev", $title) . "$product_type\">&lt;&lt; $_prev</a>\r\n";
		} else {
			$hal .= "<a href=\"" . $urlfunc->makePretty("?p=$namamodul$tambahan&$prmtr&screen=1", $title) . "$product_type\">$_first</a>\r\n";
			$hal .= "<a href=\"" . $urlfunc->makePretty("?p=$namamodul$tambahan&$prmtr&screen=$prev", $title) . "$product_type\">&lt;&lt; $_prev</a>\r\n";
		}
	} else {
	...
	if ($lastpage < 7 + ($adjacents * 2)) {   
		...
		for ($counter = 0; $counter <= $lastpage; $counter++) {
			...
			if ($counter == $screen) {
				...
				if ($prmtr == "") {
					$hal .= " <a href=\"" . $urlfunc->makePretty("?p=$namamodul$tambahan&screen=$counter", $title) . "$product_type\">$display_num</a>";
				} else {
					$hal .= " <a href=\"" . $urlfunc->makePretty("?p=$namamodul$tambahan&$prmtr&screen=$counter", $title) . "$product_type\">$display_num</a>";
				}
			}
		}
		...
		if($page < 1 + ($adjacents * 2)) {
			...
			$hal .= "<span class=\"triple-dot\">...</span>";
			if ($prmtr == "") {
				$hal .= " <a href=\"" . $urlfunc->makePretty("?p=$namamodul$tambahan&screen=$lpm1", $title) . "$product_type\">$lpm1</a>";
				$hal .= " <a href=\"" . $urlfunc->makePretty("?p=$namamodul$tambahan&screen=$lastpage", $title) . "$product_type\">$lastpage</a>";
			} else {
				$hal .= " <a href=\"" . $urlfunc->makePretty("?p=$namamodul$tambahan&$prmtr&screen=$lpm1", $title) . "$product_type\">$lpm1</a>";
				$hal .= " <a href=\"" . $urlfunc->makePretty("?p=$namamodul$tambahan&$prmtr&screen=$lastpage", $title) . "$product_type\">$lastpage</a>";
			}

		} 
		elseif($lastpage - ($adjacents * 2) > $page && $page > ($adjacents * 2)) {
			if ($prmtr == "") {
				$hal .= " <a href=\"" . $urlfunc->makePretty("?p=$namamodul$tambahan&screen=1", $title) . "$product_type\">1</a>";
				$hal .= " <a href=\"" . $urlfunc->makePretty("?p=$namamodul$tambahan&screen=1", $title) . "$product_type\">2</a>";
			} else {
				$hal .= " <a href=\"" . $urlfunc->makePretty("?p=$namamodul$tambahan&$prmtr&screen=1", $title) . "$product_type\">1</a>";
				$hal .= " <a href=\"" . $urlfunc->makePretty("?p=$namamodul$tambahan&$prmtr&screen=2", $title) . "$product_type\">2</a>";
			}
			$hal .= "<span class=\"triple-dot\">...</span>";
	...
	if($page < $counter - 1) {
		if ($prmtr == "") {
			$hal .= " <a href=\"" . $urlfunc->makePretty("?p=$namamodul$tambahan&screen=$next", $title) . "$product_type\">$_next &gt;&gt;</a>\r\n";
			$hal .= " <a href=\"" . $urlfunc->makePretty("?p=$namamodul$tambahan&screen=$lastpage", $title) . "$product_type\">$_last</a>\r\n";
		} else {
			$hal .= " <a href=\"" . $urlfunc->makePretty("?p=$namamodul$tambahan&$prmtr&screen=$next", $title) . "$product_type\">$_next &gt;&gt;</a>\r\n";
			$hal .= " <a href=\"" . $urlfunc->makePretty("?p=$namamodul$tambahan&$prmtr&screen=$lastpage", $title) . "$product_type\">$_last</a>\r\n";
			
		}
	} else {
		// $hal .= "<span class='disabled'>$_next &gt;&gt;</span>";
	}

	// $hal .= "<br><p style='margin-top:10px; margin-left: 7px'>" . _TOTAL . " $next of " . $pages . " " . _PAGESINGULAR."</p>";
    return $hal;
}
...
function fiestouploadr($fieldname, $destdir, $destfile, $maxsize, $prefname = "", $allowedtypes = "gif,jpg,jpeg,png") {
    ...
    for ($i = 0; $i <= $totalupload; $i++) {
        if ($_FILES[$fieldname]['name'][$i] != '') {
            ...
            // $uniq = uniqid();
            $thumb_name = clean_input("$prefname-$basename").'.'.$extension;
...
function showhome($posisi) {
    ...
    global $class_product_row, $class_product_item, $class_home, $mysql;
...
function base_path($string) {
	global $cfg_app_path;
	return "$cfg_app_path/$string";
}
function base_url($string) {
	global $cfg_app_url;
	return "$cfg_app_url/$string";
}
...
#region lang
function ambilterjemahan($namamodul,$id=0)
{
	global $lang, $mysqli;
	$arr = array();
	$idmenuindo = array();
	...
	{
		case "page" :
			$sql = "SELECT judul,isi FROM page_lang WHERE lang ='$lang' AND page_id='$id'";
			$result = $mysqli->query($sql);
			if($result->num_rows>0)
			{
				//echo "aaa";
				$row = $result->fetch_array();
				if($row['judul']=='' && $row['isi']=='')
				{
					$sql2 = "SELECT judul,isi FROM page WHERE id='$id'";
					$result = $mysqli->query($sql2);	
				}
				else
				{
					$sql3 = "SELECT page_id,judul,isi FROM page_lang WHERE lang='$lang' AND page_id='$id'";
					$result = $mysqli->query($sql3);					
				}
			}
			else
			{
				$sql4 = "SELECT judul,isi FROM page WHERE id='$id'";
				$result = $mysqli->query($sql4);
			}
			$row = $result->fetch_array();
			$arr = array('judul'=>$row['judul'],'isi'=>$row['isi']);
			return $arr;
		break;
		case "menu" :
			// $sql = "SELECT m.id, m.parent, m.type,(select IF(ISNULL(l.judul) or LENGTH(l.judul)<1 or count(l.judul)<1 ,m.judul,l.judul)  from menu_lang l where l.id=m.id and l.lang='$lang') judul, m.isi FROM menu m where m.id='$id'";
			
			// $kondisi = " m.id = '$id'";
			// $prefix = 'm';
			// $q = mysql_query("SELECT menu_id FROM menu_lang WHERE menu_id='$id' AND lang='$lang'");
			// if (mysql_num_rows($q) > 0) {
				// list($menu_id) = mysql_fetch_row($q);
				// if ($menu_id != '')  {
					// $kondisi = " l.menu_id = '$menu_id' AND lang='$lang'";
					// $prefix = 'l';
				// }
			// }
			
			// $sql = "SELECT m.id, m.parent, m.type, $prefix.judul FROM menu m LEFT JOIN menu_lang l ON l.menu_id=m.id WHERE $kondisi";
			// $result = mysql_query($sql);
			// $data = mysql_fetch_array($result);			
						
			// $sql = "SELECT judul FROM menu_lang WHERE menu_id='$id' AND lang='$lang'";
			// $result = mysql_query($sql);
			// $total_records = mysql_num_rows($result);
			// if ($total_records > 0) {
				// $data = mysql_fetch_array($result);
			// } else {
				// $sql = "SELECT judul FROM menu WHERE id='$id'";
				// $result = mysql_query($sql);
				// $data = mysql_fetch_array($result);
			// }
			
			$sql = "SELECT CASE WHEN lang='$lang' THEN l.judul ELSE d.judul END judul FROM menu d LEFT JOIN menu_lang l ON l.menu_id=d.id WHERE d.id='$id'";
			$result = $mysqli->query($sql);
			$data = $result->fetch_array();
			return $data['judul'];
			break;
		case "translation" :
			// $sql = "SELECT t.id, t.terjemahan,(select IF(ISNULL(tl.terjemahan) or LENGTH(tl.terjemahan)<1 or count(tl.terjemahan)<1 ,t.terjemahan,tl.terjemahan)  from translation_lang tl where tl.id=t.id and tl.lang='$lang') judul FROM translation t where t.id='$id'";
			$sql = "SELECT t.id, CASE WHEN lang='$lang' THEN l.terjemahan ELSE t.terjemahan END AS judul FROM translation t LEFT JOIN translation_lang l ON l.translation_id=t.id WHERE t.id='$id'";
		...
		break;
		case "contact":
			// $sql = "select IFNULL(ml.judulfrontend, m.judul) as judul from module_lang ml inner join module m on ml.id=m.id where  ml.lang='$lang' and m.nama='contact'";
			$sql = "select IFNULL(ml.judulfrontend, m.judul) as judul from module_lang ml inner join module m on ml.id=m.id where  ml.lang='$lang' and m.nama='contact'";
			$result = $mysqli->query($sql);
			$data = $result->fetch_array();
			return $data['judul'];	
...
function terjemahkanjudulmodul($namamodul="")
{
	global $lang, $mysqli;
	//$sql = "SELECT m.id, m.judulfrontend,(select IF(ISNULL(l.judulfrontend) or LENGTH(l.judulfrontend)<1 or count(l.judulfrontend)<1 ,m.judulfrontend,l.judulfrontend) from module_lang l where l.id=m.id and l.lang='$lang') judul FROM module m where m.nama='$namamodul'";
	$sql = "SELECT m.id FROM module m WHERE m.nama = '$namamodul'";
	$result = $mysqli->query($sql);
	$data = $result->fetch_array();
	$idmodul = $data['id'];
	$sql = "SELECT l.judulfrontend FROM module_lang l WHERE l.id = $idmodul AND l.lang = '$lang'";
	$result = $mysqli->query($sql);
	if($result->num_rows>0)
	{
		$data = $result->fetch_array();
	...
	{
		$sql = "SELECT m.judulfrontend FROM module m WHERE m.id = $idmodul";
		$result = $mysqli->query($sql);
		$data = $result->fetch_array();
	}
	return $data['judulfrontend'];
}
function bacalinkterjemahan($url,$lang)//supaya bisa baca sidebar di samping
{
	$url = trim($url);
	if(strstr($url,"menupid="))
	{
		$res = explode("menupid=",$url);
		$pid = $res[1];
		
		$res = explode("?",$res[0]);
		$url = $res[0];
		$url .= "?p=menu&pid=$pid&lang=";
	}
	else if(strstr($url, "lang"))//-->klo misalnya ada lang di dalam url, direplace isi langnya
	{
		$url = str_replace("lang=$lang","lang=",$url);
	}
	else//-->klo sudah ada: maka cek di belakangnya tanda ? ada parameternya lagi ato tidak
	{
		if(strstr($url,"?"))//klo ada concat make &
			$url .= "&";
		else
			$url .= "?";//concat make ?
		$url .= "lang=";//-->tambahkan langnya di belakang parameter
	}

	return $url;
	/*if(strstr($url,"?p="))-->jika tidak memakai sidebar disamping
	{
		if($lang == "")
		{
			$url = $url."&lang=";
...
function showTitle($modul) {
	global $lang, $mysqli;
		
	$sql = "SELECT 
			CASE WHEN lang='$lang' 
			THEN l.judulfrontend ELSE d.judulfrontend END AS judulfrontend
			FROM module d LEFT JOIN module_lang l ON l.module_id=d.id 
			WHERE nama='$modul'";
...
function showPageTranslation($pid) {
	global $lang, $mysqli;
	
	$sql = "SELECT 
			CASE WHEN lang='$lang' THEN l.judul ELSE p.judul END AS judul, 
			CASE WHEN lang='$lang' THEN l.isi ELSE p.isi END AS isi 
			FROM page p LEFT JOIN page_lang l ON l.page_id=p.id 
			WHERE p.id='$pid'";
	$result = $mysqli->query($sql);
	$response = array();
	while (list($judulhalaman, $isihalaman) = $result->fetch_row()) {
		$response = array('title' => $judulhalaman, 'content' => $isihalaman);
	}
...
function getCookieAgen($agen = 'fiesto') {
	global $mysql, $cookie_name, $config_site_hari;
	
	
	if(!isset($_COOKIE[$cookie_name])) {
		$sql = "SELECT user_id, username, telephone, cellphone1, user_email FROM webmember WHERE username='$agen' AND activity=1";
		$result = $mysql->query($sql);
		list($id, $username, $telephone, $cellphone1, $user_email) = $mysql->fetch_row($result);
		
		if ($mysql->num_rows($result) > 0) {
			setcookie($cookie_name, $username, time() + (60*60*24*$config_site_hari), "/");
			setcookie('agen_telepon', $telephone, time() + (60*60*24*$config_site_hari), "/");
			setcookie('agen_whatsapp', $cellphone1, time() + (60*60*24*$config_site_hari), "/");
			setcookie('agen_email', $user_email, time() + (60*60*24*$config_site_hari), "/");
			
		} else {
			
			$sql = "SELECT user_id, username, telephone, cellphone1, user_email FROM webmember WHERE username='fiesto' AND activity=1";
			$result = $mysql->query($sql);
			list($id, $username, $telephone, $cellphone1, $user_email) = $mysql->fetch_row($result);
			setcookie($cookie_name, $username, time() + (60*60*24*$config_site_hari), "/");
			setcookie('agen_telepon', $telephone, time() + (60*60*24*$config_site_hari), "/");
		...
		// list($id, $username, $telephone, $cellphone1, $user_email) = get1Value($username);
		// setcookie($cookie_name, $username, time() + (60*60*24*$config_site_hari), "/");
		// setcookie('telepon', $telephone, time() + (60*60*24*$config_site_hari), "/");
		// setcookie('whatsapp', $cellphone1, time() + (60*60*24*$config_site_hari), "/");
		// setcookie('email', $user_email, time() + (60*60*24*$config_site_hari), "/");
	}
	// $sql = "SELECT user_id, username, telephone, cellphone1, user_email FROM webmember WHERE username='$agen' AND activity=1";
	// echo $sql;
	// $result = $mysql->query($sql);
	...
	// if ($agen == 'fiesto') {
		// setcookie($cookie_name, $agen, time() + (60*60*24*$config_site_hari), "/");
	// } else {
		// if ($mysql->num_rows($result) > 0) {
