...
if ($action == "detail") {
    ...
    if ($result->num_rows > 0) {
        list($transaction_id ,$tanggal_buka_transaksi, $tanggal_tutup_transaksi, $customer_id, $receiver_id, $metode_pembayaran, $shipping, $status, $catatan, $no_pengiriman, $latestupdate, $kodevoucher, $tipe_voucher, $nominal_voucher, $packingvalue, $insurancevalue, $kurir, $is_dropship) = $result->fetch_row();
    ...
    if (empty($trns)) {
		...
		if ($result->num_rows > 0) {
			while($row = $result->fetch_assoc()) {
				...
				$res_catalog = $mysqli->query($sql_catalog);
				while(list($id, $restock, $is_stock) = $res_catalog->fetch_row()) {
					if (!$is_stock) {
    ...
    if ($result->num_rows == 0) {
        ...
        while ($row = $result->fetch_assoc()) {
			...
			if ($row['is_dropship'] == 1) {
				...
				$resultcustomername = $mysqli->query($sqlcustomername);
...
//            $admincontent .= "<p><a class=\"buton\" href=\"?p=order\">" . _GOTOMAIN . " $judulmodul</a></p>\r\n";
        ...
        while ($row = $result->fetch_assoc()) {
            ...
            $resultnama = $mysqli->query($sqlnama);
            $rowsql = $resultnama->fetch_assoc();
            $admincontent .= "		<td>" . tglformat($row['tanggal_buka_transaksi']) . "</td>\r\n";
...
if ($action == 'invoice') {
    ...
    $result = $mysqli->query($sql);
    if ($result->num_rows > 0) {
        list($transaction_id ,$tanggal_buka_transaksi, $tanggal_tutup_transaksi, $customer_id, $receiver_id, $metode_pembayaran, $shipping, $status, $catatan, $no_pengiriman, $latestupdate, $kodevoucher, $tipe_voucher, $nominal_voucher, $kurir) = $result->fetch_row();
		$couriers = json_decode($kurir, true);
		$courier_tipe = $couriers['tipe'];
		$courier_city = explode(',', $couriers['kota']);
		$kecamatan = $courier_city[0];
        ...
        $result = $mysqli->query($sql);
		$i = 1;
        while (list($quantity, $price, $productname, $filename, $idmerek) = $result->fetch_row()) {
			$brand_name = get_brand_name($idmerek);
            $line_cost = $quantity * $price;  //work out the line cost
            $total = $total + $line_cost;   //add to the total cost
            $admincontent .= "<tr>";
	...
	if ($result = $mysqli->query($sql)) {
		...
		if ($total_records == 0) {
			...
			while($row = $result->fetch_assoc()) {
				...
				list($fullname, $user_email, $address1, $address2, $city, $state, $zip, $telephone, $nama_pengirim, $telp_pengirim) = $result_customer->fetch_row();
				
				$sql_detail = "SELECT product_name, jumlah, harga FROM sc_transaction_detail WHERE transaction_id='$order_code'";
				$qty_arr 		= array();
				$priceidr_arr 	= array();
				$pesanan_arr 	= array();
				if ($res_detail = $mysqli->query($sql_detail)) {
					while($row_detail = $res_detail->fetch_assoc()) {
