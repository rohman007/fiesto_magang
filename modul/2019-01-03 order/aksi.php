...
if ($action == '' || $action=='view') {
	...
	$result = mysql_query($sql);
	list($tanggal_buka_transaksi, $tanggal_tutup_transaksi, $customer_id, $receiver_id, $metode_pembayaran, $shipping, $status, $catatan, $no_pengiriman, $latestupdate, $kodevoucher, $tipe_voucher, $nominal_voucher, $packingvalue, $insurancevalue, $kurir) = mysql_fetch_row($result);
	...
	$result = mysql_query($sql);
	while (list($quantity, $price, $productname,$filename, $idmerek, $attribut_id) = mysql_fetch_row($result)) {
	...
	$result = mysql_query($sql);
	list($nama_lengkap, $email, $alamat, $alamat2, $kota, $provinsi, $zip, $telepon, $cellphone1, $cellphone2, $sendername, $senderphone, $senderemail) = mysql_fetch_row($result);
	...
	$result = mysql_query($sql);
	list($logo,$bank,$cabang,$norekening,$atasnama) = mysql_fetch_row($result);
	...
	$content .= "<table border=\"1\" cellpadding=\"3\" style=\"margin-top:15px;color:#000;border-collapse: collapse;border: 1px solid #bbb;\" class=\"table table-bordered\" width=\"100%\">\r\n";
	$content .= "<tr style=\"background: #eee;font-weight: bold;\">";
	if ($pakaiongkir) $content .= "<th>"._POSTAGE."</th>\r\n";
...
/* if ($action == 'history') {
	...
	if ($_SESSION['member_uid']!='') {
		...
		while (list($txid, $txdate, $status, $ongkir,$no_pengiriman, $status_pembayaran) = mysql_fetch_row($result)) {
...
if ($action == 'history') {
	...
	if ($_SESSION['member_uid']!='') {
		...
		while (list($txid, $txdate, $status, $ongkir,$no_pengiriman, $status_pembayaran, $nominal, $tanggal_konfirmasi, $kodevoucher, $tipe_voucher, $nominal_voucher, $kurir) = mysql_fetch_row($result)) {
			$sql1 = "SELECT jumlah, harga FROM sc_transaction_detail WHERE transaction_id='$txid'";
			$result1 = mysql_query($sql1);
			$total = 0;
			while (list($quantity, $price) = mysql_fetch_row($result1)) {
				// $total = $total + $qty * $harga;
...
if ($action == 'trackorder'){
		...
		while (list($txid, $txdate, $status, $ongkir,$no_pengiriman) = mysql_fetch_row($result)) {
			$sql1 = "SELECT jumlah, harga FROM sc_transaction_detail WHERE transaction_id='$txid'";
			$result1 = mysql_query($sql1);
			$total = 0;
			while (list($qty, $harga) = mysql_fetch_row($result1)) {
...
if ($action == 'confirm_payment') {
	...
	$result = mysql_query($sql);
	list($tanggal_buka_transaksi, $tanggal_tutup_transaksi, $customer_id, $receiver_id, $metode_pembayaran, $shipping, $status, $catatan, $no_pengiriman, $latestupdate, $kodevoucher, $tipe_voucher, $nominal_voucher, $packingvalue, $insurancevalue, $kurir, $tanggal_konfirmasi, $nominal) = mysql_fetch_row($result);
