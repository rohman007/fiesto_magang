...
if ($action == "save") {
    if (isset($_POST['submit'])) {
        ...
        if ($notificationbuilder != "") {
            ...
            $sql = "INSERT INTO testi (nama, judul, summary, keterangan, publish, ishot) VALUES ('$nama', '$judul', '$summary', '$keterangan', '$publish', '$ishot')";
	...
	<form class="form-horizontal" method="POST" action="' . $thisfile . '" enctype="multipart/form-data">
	  <input type="hidden" name="action" value="save">
		  ...
		  <div class="control-group">
	        ...
	        <div class="controls"><input type="text" name="judul" size="40" value=' . $judul . '> </div>
	      </div>
		  <div class="control-group">
		  ...
		  <div class="control-group">
			...
			<div class="controls"><input type="checkbox" name="ishot" value="1" checked></div>
		  </div>
		  <!--<div class="control-group">
...
if ($action == "update") {
    if (isset($_POST['submit'])) {
        ...
        if ($notificationbuilder != "") {
			...
			judul='$judul', summary='$summary', keterangan='$keterangan', ishot='$ishot', publish='$publish' WHERE id='$pid'";
            } else {
                $sql = "UPDATE testi SET nama='$nama', 
			judul='$judul', publish='$publish', summary='$summary', keterangan='$keterangan', ishot='$ishot' WHERE id='$pid'";
            }
            $result = $mysql->query($sql);
            if ($result) {
                $action = createmessage(_DBSUCCESS, _SUCCESS, "success", "");
...
if ($action == "modify") {
    ...
    $sql = "SELECT  id, filename, nama, judul, publish, summary, keterangan, ishot FROM testi WHERE id='$pid'";
    $result = $mysql->query($sql);

    if ($mysql->num_rows($result) > 0) {
//        $publishstatus = ($publish == 1) ? 'checked' : '';
        list($id, $filename, $nama, $judul, $publish, $summary, $keterangan, $ishot) = $mysql->fetch_row($result);
    ...
    <form class="form-horizontal" method="POST" action="' . $thisfile . '" enctype="multipart/form-data">
        ...
        <div class="control-group">
		  ...
		  <div class="control-group">
			<label class="control-label">Hot</label>
			<div class="controls"> <input type="checkbox" name="ishot" value="1" ' . $hotchecked . '/></div>
		  </div>
		  <!--<div class="control-group">
			<label class="control-label">' . _PUBLISHED . '</label>
...
if ($action == "") {
    ...
    if ($mysql->num_rows($result) == "0") {
        ...
        while (list($id, $nama, $ishot) = $mysql->fetch_row($result)) {
