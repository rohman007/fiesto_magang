...
//jika berhasil, baru add record
                $sql = "INSERT INTO bannerdata (cat_id, title, url, publish, urutan) VALUES ('$cat_id', '$bannertitle', '$bannerurl', '$publish', '$urutan')";
                if ($mysql->query($sql)) {
...
//del gambar yang dioverwrite (hanya jika filename beda)
                ...
                $sql = "UPDATE bannerdata SET cat_id='$cat_id', title='$bannertitle', url='$bannerurl', publish='$publish', urutan='$urutan', filename='$modifiedfilename' WHERE id='$pid'";
            } else {
                $sql = "UPDATE bannerdata SET cat_id='$cat_id', title='$bannertitle', url='$bannerurl', publish='$publish', urutan='$urutan' WHERE id='$pid'";
            }
            $result = $mysql->query($sql);
            if ($result) {
                $action = createmessage(_DBSUCCESS, _SUCCESS, "success", "viewcat");
...
if ($action == "savecat") {
    if (isset($_POST['submit'])) {
        ...
        if ($notificationbuilder != "") {
            ...
            $sql = "INSERT bannercat (nama, urutan) values ('$nama','$mx')";
            $result = $mysql->query($sql);
            $mid = $mysql->insert_id();
...
if ($action == "updatecat") {
    if (isset($_POST['submit'])) {
        ...
        if ($notificationbuilder != "") {
            ...
            $sql = "UPDATE bannercat set nama='$nama' WHERE id='$cat_id'";
            $result = $mysql->query($sql);
...
if ($action == "viewcat") {
    ...
    $result = $mysql->query($sql);
    list($admintitle) = $mysql->fetch_row($result);
    if ($mysql->num_rows($result) == "0") {
