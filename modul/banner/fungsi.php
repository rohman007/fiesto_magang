...
function show_me_the_images() {
	...
	while (list($photo_id, $photo_cat_id, $photo_filename, $photo_title, $photo_published, $photo_ishot) = mysql_fetch_row($photo)) {
...
function show_me_the_list() {
	...
	$intro = $mysql->query("SELECT value FROM config WHERE name='bannerlistintro'");
	$show_intro = $mysql->fetch_array($intro);
	$catcontent .= "$show_intro[0]";
	
	$catcontent .= "<div class=\"clientdiv\">";
	$catcontent .= "<ul class=\"clientul\">";
	while (list($id, $title, $filename, $publish, $url) = $mysql->fetch_row($photo)) {
		$titleurl = array();
		$titleurl["pid"] = $photo_title;
