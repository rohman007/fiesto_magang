        ...
        if ($value != null || $value != "") {
            ...
            if (in_array(0, $values)) {
				...
				if (mysql_num_rows($result) > 0) {
					...
					while (list($id, $kota, $ongkos, $ongkostetap, $waktukirim) = mysql_fetch_row($result)) {
						$cityfound++;
						/* -------- PERHITUNGAN ONGKIR -------- */
            ...
            // $result = mysql_query($sql);
            // while (list($courier_id) = mysql_fetch_row($result)) {
                // if (in_array($courier_id, $values)) {
                    ...
                    // while (list($id, $nama, $asal, $kota, $tipe, $waktu_kirim, $biaya) = mysql_fetch_row($result)) {
                        // $cityfound++;
                        // /* -------- PERHITUNGAN ONGKIR -------- */
           ...
           if (in_array(1, $values)) {
			...
			if (in_array(2, $values)) {
				...
				if (mysql_num_rows($result) > 0) {
					...
					while (list($kode, $kota, $kgp, $kgs, $bppw, $bpp, $bpsw, $bps, $addrp) = mysql_fetch_row($result)) {
						$cityfound++;
						/* -------- PERHITUNGAN ONGKIR -------- */
