...
function showcart() {
	...
	global $pakaicatalogattribut, $catalog_attribute_option;
    ...
    if ($pakaicart) {
        ...
        if (!empty($_SESSION['cart'])) {
			...
			list($city) = mysql_fetch_row($result);
			...
			foreach ($_SESSION['cart'] as $product_id => $quantity) {
				...
				if (mysql_num_rows($result) > 0) {
					list($pid, $cat_id, $title, $matauang, $hargadiskon, $filename, $brand_id, $product_code) = mysql_fetch_row($result);
					...
					if ($pakaicatalogattribut) {
						...
						switch($catalog_attribute_option) {
							...
							case 1:
								if ($product_size != '') {
									$content .= '<small class="product-attribut">'.$product_size.'</small>';
								}
								break;
							case 2:
								if ($product_color != '') {
									$content .= '<small class="product-attribut">'.$product_color.'</small>';
								}
								break;
						}						
					}
			...
			if ($pakaivoucher) {
				...
				if ($_POST['submitcode'] != '' && !isset($_SESSION['voucher'])) {
					...
					if (mysql_num_rows($qres) > 0) {
						$voucher = mysql_fetch_array($qres);
...
function hitung_berat() {
    ...
    foreach ($_SESSION['cart'] as $product_id => $qty) {
        ...
        if ($product_id != '') {
            ...
            $result = mysql_query($sql);
            if (mysql_num_rows($result) > 0) {
                list($id, $description, $productname, $price, $normalprice, $weight) = mysql_fetch_row($result);
...
function info_beli($cityReceiver) {
    ...
    if ($pakaiongkir) {
        ...
        if ($pakaiongkir) {
            ...
            $totalshipping = 0;
            $result = mysql_query($getongkosquery);
            if (mysql_num_rows($result) > 0) {
...
function get_biaya_ongkir($kota_tujuan, $berat = 1, $tipe = 'REG') {
	...
	$result = mysql_query($sql);
	if (mysql_num_rows($result) > 0) {
		list($biaya) = mysql_fetch_row($result);
		return $biaya * $berat;
	}
	return false;
}
