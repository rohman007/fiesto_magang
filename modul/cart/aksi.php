...
if($action == 'add') 
{
	...
	if ($pakaicatalogattribut) {
		...
		$param_color=fiestolaundry($_POST['paramcolor'],100);
	}
	...
	if ($pakaifbpixel) {
		$content_name = fiestolaundry($_POST['content_name'], 50);
		$content_category = fiestolaundry($_POST['content_category'], 50);
		$content_ids = fiestolaundry($_POST['content_ids'], 50);
		$value = fiestolaundry($_POST['value'], 50);
		...
		$params_attr['id'] = $content_ids;
		$params_attr['quantity'] = $quantity;
		$params_attr['item_price'] = $value;
		$params_attr['design'] = $attr;
		$params_attr['note'] = $_POST['is_notes'];
		$params['contents'] = array($params_attr);
			
		$fb_param_pixels = json_encode($params);
		$content .= <<<HTML
		<script>
		  fbq('track', 'AddToCart', $fb_param_pixels);
		</script>
HTML;
	}
...
*/
			...
			<input type=\"hidden\" name=\"usr_id\" id=\"usr_id\" value=\"$id\" />
                            ...
                            <fieldset>
                                    ...
                                    <input type=\"hidden\" name=\"usr_id\" id=\"receive_usr_id\" value=\"$id\" />
									<input class=\"form-control\" type=\"hidden\" name=\"email\" value=\"$emaildb\" />
			...
			if ($pakaiongkir) {
				...
				$result = $mysql->query($sql);
				list($idcouriers) = $mysql->fetch_row($result);
		...
		// HTML;
					...
					foreach ($_SESSION['cart'] as $product_id => $quantity) {
						...
						if ($result && $mysql->num_rows($result) > 0) {
							list($title2, $matauang, $hargadiskon, $filename, $brand_id) = $mysql->fetch_row($result);
...
if ($action == "order") {
    if (!empty($_SESSION['cart'])) {
        ...
        if ($isvalid) {
            ...
            $qShowStatusResult = $mysql->query($qShowStatusCustomer);
            ...
            $qShowStatusResult = $mysql->query($qShowStatusReceiver);
            $row = $mysql->fetch_assoc($qShowStatusResult);
...
// //            $transactionIncrement = $transId;
			...
			$qShowStatusResult = $mysql->query($qShowStatusTranscation);
			$row_result = $mysql->fetch_assoc($qShowStatusResult);
            ...
            $qShowStatusResult = $mysql->query($qShowStatusTranscationDetail);
            $row = $mysql->fetch_assoc($qShowStatusResult);
            ...
            $sql = "INSERT INTO sc_customer(nama_lengkap, web_user_id, email, alamat, alamat2, 
                ...
                VALUES ('$fullname','$web_user_id','$email','$address','$address2','$city', 
                '$state','$zip','$phone','$cellphone1','$cellphone2')";
            $resultCustomer = $mysql->query($sql);
            if ($resultCustomer) { //$resultCustomer success
                ...
                $resultReceiver = $mysql->query($sql);
                if ($resultReceiver) {
                    ...
                    $resultTransaction = $mysql->query($sql);
                    if ($resultTransaction) { //$resultTransaction success 
                        ...
                        foreach ($_SESSION['cart'] as $product_id => $quantity) {
                            ...
                            if ($product_id != '') {
                                ...
                                $result = $mysql->query($sql);
                                list($hargadiskon, $productname, $filename) = $mysql->fetch_row($result);
								...
								$resultTranscationDetail = $mysql->query($sql);
								
								if ($pakaistok) {
									$sql_stock = "UPDATE catalogdata SET stock=stock-$quantity WHERE id='$product_id'";
									$result_stock = $mysql->query($sql_stock);
								}
                            }
                        }
						...
						if ($voucher_onetime == 1) {
							$q1 = $mysql->query("UPDATE voucher SET status=0 WHERE id='$voucher_id'");
						}
						$q2 = $mysql->query("INSERT INTO voucher_detail (voucher_id, member_id) VALUES ('$voucher_id', '$member_id') ");
...
// STYLE;
                ...
                $htmlmsg .= "<tr class=\"header-table\" style=\"font-weight: bold;\">";
                ...
                foreach ($_SESSION['cart'] as $product_id => $quantity) {
                    ...
                    if ($product_id != '') {
                        ...
                        if ($mysql->num_rows($result) > 0) {
                            ...
                            list($id, $description, $productname, $price, $normalprice, $weight, $filename, $idmerek) = $mysql->fetch_row($result);
                            ...
                            $htmlmsg .= "<td style=\"padding: 20px 20px;border-bottom: 1px solid #aaa;\"><span style=\"display:none\">" . _THUMBNAIL . "</span><img src=\"$cfg_thumb_url/$filename\" style=\"width:50px;\" ></td>";
                            $htmlmsg .= "<td style=\"padding: 20px 20px;border-bottom: 1px solid #aaa;font-weight:bold\"><span style=\"display:none\">" . _PRODUCTNAME . "</span><small style=\"font-size: 15px;font-weight: bold;color: #000000;line-height: 1.5;text-transform:uppercase\" class=\"brand-name\">$brand_name</small><p style=\"line-height: 1.45;margin-bottom: 0;margin-top: 5px;line-height: 1.45;margin-bottom: 0;margin-top: 5px;\" class=\"product-name\">$productname</p>";
							if ($pakaicatalogattribut) {
								...
								switch($catalog_attribute_option) {
									case 0:
										$product_attr = "$product_size, $product_color";
										break;
									case 1:
										$product_attr = $product_size;
										break;
									case 2:
										$product_attr = $product_color;
										break;
								}
								
								$htmlmsg .= '<small class="product-attribut">'.$product_attr.'</small>';
							}
							$htmlmsg .= '</td>';
                            $htmlmsg .= "<td style=\"padding: 20px 20px;border-bottom: 1px solid #aaa;\" align=\"right\"><span style=\"display:none\">" . _PRICE . "</span>" . number_format($price, 0, ',', '.') . "</td>";
                            //along with a 'remove' link next to the quantity - which links to this page, but with an action of remove, and the id of the current product
                            $htmlmsg .= "<td style=\"padding: 20px 20px;border-bottom: 1px solid #aaa;\" align=\"center\"><span style=\"display:none\">" . _QUANTITY . "</span>" . number_format($quantity, 0, ',', '.') . "</td>";
                            $htmlmsg .= "<td style=\"padding: 20px 20px;border-bottom: 1px solid #aaa;\" align=\"right\"><span style=\"display:none\">" . _LINECOST . "</span>" . number_format($line_cost, 0, ',', '.') . "</td>";
                            $htmlmsg .= "</tr>";
                        }
                    }
                }
				...
				$htmlmsg .= "<td class=\"label-table\" colspan=\"4\" align=\"right\" style=\"font-weight: bold;color: #999;padding:20px 20px 6px\">"._SUBTOTAL."</td>";
				...
				if ($pakaivoucher) {
					...
					$htmlmsg .= "<tr class=\"noborder\">";
					$htmlmsg .= "<td class=\"label-table\" colspan=\"4\" align=\"right\" style=\"font-weight: bold;color: #999;padding:20px 20px 6px\">"._DISCOUNT."</td>";
				...
				if ($pakaiadditional) {
					...
					$htmlmsg .= "<tr class=\"noborder\">";
					$htmlmsg .= "<td class=\"label-table\" colspan=\"4\" align=\"right\" style=\"font-weight: bold;color: #999;padding:20px 20px 6px\">"._INSURANCE."</td>";
					...
					$htmlmsg .= "<tr class=\"noborder\">";
					$htmlmsg .= "<td class=\"label-table\" colspan=\"4\" align=\"right\" style=\"font-weight: bold;color: #999;padding:20px 20px 6px\">"._PACKING."</td>";
                ...
                if ($pakaiongkir) {
					...
					// if (mysql_num_rows($result) > 0) {
						...
						$htmlmsg .= "<td class=\"label-table\" colspan=\"4\" align=\"right\" style=\"font-weight: bold;color: #999;padding:20px 20px 6px\">"._POSTAGE."</td>";
                ...
                $htmlmsg .= "<td class=\"label-table\" colspan=\"4\" align=\"right\" style=\"background: #fff;padding: 14px 20px;margin-top: 10px;border-top: 20px solid #f3f3f3;font-size: 16pt;color: #000 !important;font-weight:bold\">" . _TOTAL . "</td>";
                $htmlmsg .= "<td align=\"right\" style=\"background: #fff;padding: 14px 20px;margin-top: 10px;border-top: 20px solid #f3f3f3 ;font-size: 16pt;color: #000 !important;font-weight:bold\">" . number_format($total, 0, ',', '.') . "</td>";
                $htmlmsg .= "</tr>";
                $htmlmsg .= "</table></div>";
				
				// $htmlmsg_customer_email = "";
				$htmlmsg_customer_email = $htmlmsg;
				// $htmlmsg_customer_email .= "</div>";
               ...
               if (!$iscityfound)
				...
				$strArr = array(
					'/#hargatotal/' => "Rp" . number_format($total, 0, ',', '.'),
				...
				$htmlmsg_content_email = "<html>
				  <head>
					<style>
					  @media screen and (max-width: 500px) {
						 .checkout-form > h3{margin-top:40px;}
						.checkout-form table,.checkout-form tbody,.checkout-form tr,.checkout-form td{display:block;width:100% !important;box-sizing:border-box} 
						.list-order-product tr.header-table{display:none}
						.list-order-product {width:100%;}
						.list-order-product td{border-bottom:1px solid #eee !important;text-align:left !important;padding:10px 5px !important; }
						.list-order-product td span {
							position: relative;
							margin-right: 4px;
							font-weight:bold;
							margin-bottom: 3px;
						...
						.checkout-form .list-order-product tr.noborder td {width:48% !important;border-bottom:0px !important;display:inline-block !important;text-align:right !important}
						.checkout-form .list-order-product tr.noborder.total_order td {width:48% !important;border-top:0px !important;display:inline-block !important;font-size:14pt !important;margin:0 !important}
						.checkout-form .list-order-product tr.noborder td.label-table{text-align:left !important}
						.list-order-product tr {
				...
				$htmlmsg_content_email .= "<h3 style=\"color:#000\">"._ORDERDETAILS."</h3>\r\n";
				$htmlmsg_content_email .= $htmlmsg_customer_email;
				$htmlmsg_content_email .= "</div></body></html>";
                ...
                $resultTranscationUpdate = $mysql->query($sql);
				...
				if ($pakaifbpixel) {
					...
					$content .= <<<SCRIPT
					<script>
						fbq('track', 'CompleteRegistration', $fb_param_pixels);
...
if ($action == 'review_order') {
	...
	if (!empty($_SESSION['cart'])) {
        ...
        if ($isvalid) {
                ...
                foreach ($_SESSION['cart'] as $product_id => $quantity) {
                    ...
                    if ($product_id != '') {
                        ...
                        if ($mysql->num_rows($result) > 0) {
                            ...
                            list($id, $description, $productname, $price, $normalprice, $weight, $filename, $diskon, $matauang, $nilaidiskon, $idmerek) = $mysql->fetch_row($result);
			...
			$content .= "<form action=\"".$urlfunc->makePretty("?p=cart&action=order")."\" method=\"POST\"  class=\"checkout-form\">";
			$content .='<div id="checkout-form">';
...
HTML;
			...
			$content .= '
				<label>
				  <input type="checkbox" name="terms" value="1" required><span class="checkbox-material">
			...
			$content .= "<p>&nbsp;</p>";            
			$content .= "<div class=\"biodatabawah\"><a class=\" link_kembali\" href=\"javascript:history.back();\"><i class=\"fa fa-angle-left fa-3\"></i>"._BACK."</a>&nbsp;&nbsp;&nbsp;<input class=\"btn btn-default more\" type=\"submit\" name=\"checkout\" value=\""._COMPLETEORDER."\" /></div><br />";
			$content .= "</form>";            
