...
if ($action == "images") 
{
    ...
    if (!empty($cat_id)) 
    {
        ...
        $result = mysql_query($sql);
        list($catname, $catdesc, $catfilename) = mysql_fetch_row($result);
        ...
        if (mysql_num_rows($result) > 0) 
        {
            ...
            $result = mysql_query($sql);
			list($cat_name) = mysql_fetch_row($result);
            ...
            if ($totalsubcat > 0) 
            {
                ...
                while (list($catsubcat_id, $catsubcat_name, $catsubcat_filename) = mysql_fetch_row($result)) 
                {
...
if (substr($action, 0, 4) == 'view') {
	...
	//ukuran	
	$catcontent .='<p>';
	$q=mysql_query("SELECT size,stok from attribut WHERE id='$pid' order by auto");
	if(mysql_num_rows($q))
	{	$i=0;
    ...
    for ($i = 0; $i < $num; $i++) {
        ...
        $titleurl = array();
                ...
                if ($action == 'viewimages') {
					...
					list($catname)=mysql_fetch_row($result1);
...
if ($action == "brand") {
    ...
    if ($total_images == "0") {
		...
		$result = mysql_query($sql);
		list($namamerek) = mysql_fetch_row($result);
		if ($pages > 1) {
			$titleurl = array();
...
if($action=="detail") {
	...
	$sql = "SELECT id, cat_id, filename, title, publish, ishot, ketsingkat, keterangan,
			...
			as nilaidiskon,
			harganormal-(if(LOCATE('%',diskon)>0,(TRIM(REPLACE(diskon,'%',''))/100*harganormal),
				diskon)) as hargadiskon,thumb,isbest,ispromo,isnew,issold,idmerek
			FROM catalogdata WHERE id='$pid'";
	...
	$atm=mysql_query("SELECT id,nama,type FROM catalog_atm order by id");
	if($atm and mysql_num_rows($atm)>0) {
		$x=0;
		$content_attribut="";
		while($datm=mysql_fetch_assoc($atm)) {
	...
	if ($hasil['harganormal'] > 0) {
		...
		if ($pakaicart and !$hasil['issold']) {
			...
			if ($pakaicatalogattribut) {
				$sql = "SELECT id, size, color, hexa, price FROM catalogattribut WHERE catalog_id='$pid' GROUP BY size ORDER BY urutan";
			...
			$content .= '<div class="input-colom"><a id="qtyminus" class="qty-trigger qty-trigger-detail" data-id="'.$pid.'"><i class="fa fa-minus"></i></a>';
			if ($pakaistock) {
			...
			$content .= '<a id="qtyplus" class="qty-trigger qty-trigger-detail" data-id="'.$pid.'"><i class="fa fa-plus"></i></a>';
