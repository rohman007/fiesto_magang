...
if ($action == "catdel") {
	if (empty($cat_id)) {
		...
		if ($number > 0) {
			...
			$result = mysql_query($sql);
			$catsubcatnumber=mysql_num_rows($result);
...
if ($action == "delete_cat_confirmed") {
	...
	if (mysql_num_rows($result) == "0") {
		...
		list($cid,$cparent,$cnama,$cdescription,$curutan) = mysql_fetch_row($result);
		if ($cparent == "0") {	
...
if (($action=="add") || ($action=="edit")) {
	...
	if ($action=="edit") {
		...
		$sql = "SELECT id, filename, date, title,idmerek, ketsingkat, keterangan, harganormal, diskon,thumb, sku, url $tambahan $sc_field FROM catalogdata WHERE id='$pid'";
	...
	*/
	...
	if ($pakaicatalogattribut) {
		...
		if($action=='edit') {
			...
			$q=mysql_query("SELECT id,size,price,color,hexa,urutan FROM catalogattribut WHERE catalog_id='$pid' ORDER BY urutan ASC");
	...
	// echo "<div>";
		...
		// {
		// echo "<div id=\"product_ukuran_group\" style=\"line-height:30px;\">";
		// $q=mysql_query("select id,size,stok from attribut where id='$pid' order by id asc");
	...
	// //end ukuran
	// //attribut tambahan
	// $q=mysql_query("SELECT * from catalog_atm limit 1");
	...
	if($sukses) {
		...
		if (mysql_query($sql)) {
			...
			if($prod_attribut==1) {
				...
				$stok=$_POST['stok'];
				if(count($ukuran>0) and count($stok)>0) {
					if (!mysql_query("DELETE FROM attribut WHERE id='$newid'")) $action = createmessage(_DBERROR, _ERROR, "error", "");	//pesan(_ERROR,_DBERROR);
					
					foreach($ukuran as $i =>$u) {
						if(strlen($ukuran[$i])>0 and strlen($stok[$i])>0) {
							if (!mysql_query("INSERT INTO  attribut(id,size,stok) values ('$newid','".$ukuran[$i]."','".$stok[$i]."')")) pesan(_ERROR,_DBERROR);
						}
					}
				}
			}
		...
		if($sukses) {
			...
			if($totalupload>0) {
				...
				if(count($namathumbnail)>0) {
					...
					if (!mysql_query($sql)) $action = createmessage(_DBERROR, _ERROR, "error", "");//pesan(_ERROR,_DBERROR);
				}
...
if ($action=="ubahdata") { 
	...
	if($prod_attribut==1) {
		...
		$stok=$_POST['stok'];
		if(count($ukuran>0) and count($stok)>0) {
			if (!mysql_query("DELETE FROM attribut WHERE id='$pid'")) pesan(_ERROR,_DBERROR);
		
			foreach($ukuran as $i =>$u) {
				if(strlen($ukuran[$i])>0 and strlen($stok[$i])>0) {
					if (!mysql_query("INSERT INTO  attribut(id,size,stok) values ('$pid','".$ukuran[$i]."','".$stok[$i]."')")) pesan(_ERROR,_DBERROR);
				}
			}
		}
	}
	...
	if($totalupload>0) {
		...
		if(count($namathumbnail)>0) {
			...
			// if (!mysql_query($sql)) pesan(_ERROR,_DBERROR);
	...
	if (!mysql_query($sql)) $action = createmessage(_DBERROR, _ERROR, "error", "");	
	} */
	//
	if (mysql_query($sql)) {
		$action = createmessage(_DBSUCCESS, _SUCCESS, "success", "");	
...
if ($action=="images") {
	...
	$result = mysql_query($sql);
	$total_subcat = mysql_num_rows($result);
	if ($total_subcat > 0) 
	{
...
if (($action=="delete") || ($action=="confirm_delete")) {
	...
	if ($action=="delete") {
		...
		$getfilename=mysql_query($sql);
		list($cat_id,$filename,$thumb) = mysql_fetch_row($getfilename); 
		...
		if (mysql_query($sql)) {
			if (!mysql_query("DELETE FROM catalogattribut WHERE catalog_id='$pid'")) pesan(_ERROR,_DBERROR);
			$action = createmessage(_DBSUCCESS, _SUCCESS, "success", "");	
		} else {
			$action = createmessage(_DBERROR, _ERROR, "error", "");
		}
...
if ($action == "catnew") {
	...
	*/
	...
	list($catlist_name) = mysql_fetch_row($catlist);
...
if ($action == "catedit") {
	...
	if (empty($cat_id)) {
		...
		$cat = mysql_query($sql);
		list($cat_name, $cat_parent_id,$cat_desc, $url) = mysql_fetch_row($cat);
		...
		$hasil = mysql_fetch_assoc($query_result);
		if($hasil['filename']!='') {
...
if ($action=='bulkedit') {
	...
	if ($_POST['submit']==_SAVE) {
		...
		for ($i=0;$i<count($_POST['bulkid']);$i++) {
			...
			if (!mysql_query("UPDATE catalogdata SET title='".addslashes($_POST['bulktitle'][$i])."', weight='".$_POST['bulkweight'][$i]."', harganormal='".$_POST['bulkharganormal'][$i]."', diskon='".$_POST['bulkdiskon'][$i]."', ishot='".$_POST['bulkishot'][$i]."', publish='$published' $sc_field  WHERE id='".$_POST['bulkid'][$i]."'")) pesan(_ERROR,_DBERROR);	//jun masalah
		}
...
if ($action=='attribut_save') {
	...
	foreach($nama as $i =>$v) {
		if($idattr[$i]!='') {
			if($v!='') {
			$q=mysql_query("UPDATE catalog_atm SET nama='".$v."' WHERE id='".$idattr[$i]."'");
			$q=mysql_query("DELETE FROM catalog_atd WHERE id_atm='".$idattr[$i]."'");
				if($type_attribut[$i]==2) {
					//dropdown
					$atr_value=explode(",",$attribut_value[$i]);
					foreach($atr_value as $x => $y) {
						if($y!='') {
							$q1=mysql_query("INSERT INTO catalog_atd(id_atm,value) values ('".$idattr[$i]."','".$y."')");
							if($q1) {
				...
				$q=mysql_query("DELETE FROM catalog_atm WHERE id='".$idattr[$i]."'");
				$q=mysql_query("DELETE FROM catalog_atd WHERE id_atm='".$idattr[$i]."'");
			}
		} else {
			if($v!='') {
				list($lastid)=mysql_fetch_row(mysql_query("select max(id) from catalog_atm"));
				$lastid=$lastid+1;
				$q=mysql_query("INSERT INTO catalog_atm(id,nama,type) values ('$lastid','".$v."','".$type_attribut[$i]."')");
				if($q) {
					if($type_attribut[$i]==2) {
						//dropdown
						$atr_value=explode(",",$attribut_value[$i]);
						foreach($atr_value as $x => $y) {
							if($y!='') {
								$q1=mysql_query("INSERT INTO catalog_atd(id_atm,value) values ('$lastid','".$y."')");
								if($q1) {
									pesan(_SUCCESS,_SIMPANATTRIBUTBERHASIL);
								}
							}
...
if ($action=='attribut_add')  {
	...
	while($d=mysql_fetch_assoc($res)) {
		$detail_value="";
		if($d['type']==2) {
			$res1=mysql_query("SELECT group_concat( value SEPARATOR ',') value  FROM catalog_atd WHERE id_atm='".$d['id']."'");
			list($detail_value)=mysql_fetch_row($res1);
		}
