...
if ($jenis=='featured') {
	...
	if (mysql_num_rows($result)>0) {
		...
		while (list($photo_id, $photo_cat_id, $photo_filename, $photo_title, $photo_cat_name) = mysql_fetch_row($result)) {
			$titleurl = array();
...
if ($jenis=='productlist') {
	...
	for ($i=0; $i<count($cats->cats); $i++) {
		...
		$resultproduk = mysql_query($sqlproduk);
		if(mysql_num_rows($resultproduk)>0)
		{	$widget .= "<ul>\r\n";
			while(list($photo_id,$nama) = mysql_fetch_row($resultproduk)) 
			{	$titleurl = array();
				$titleurl["pid"] = $nama;
				$titleurl["cat_id"] = $cats->cats[$i]['nama'];
