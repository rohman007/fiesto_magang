...
function hitungtotal($txid) {
	...
	$total = 0;
	while (list($jumlah,$harga,$attributId) = $mysql->fetch_row($result)) {
	...
	$result = $mysql->query($sql);
	list($ongkir, $kodevoucher, $tipe_voucher, $nominal_voucher) = $mysql->fetch_row($result);
...
function sendemailstatus($txid) {
	global $mysql, $pakaicatalogattribut, $catalog_attribute_option, $smtpsendername;
	...
	$result = $mysql->query($sql);
	list($tanggal_buka_transaksi, $tanggal_tutup_transaksi, $customer_id, $receiver_id, $metode_pembayaran, $shipping, $status, $catatan, $no_pengiriman, $latestupdate, $kodevoucher, $tipe_voucher, $nominal_voucher, $kurir, $is_dropship) = $mysql->fetch_row($result);
...
STYLE;
	...
	if ($is_dropship == 1) {
		...
		$result = $mysql->query($sql);
		list($web_user_id, $nama_lengkap_d, $email_d, $cellphone1_d) = $mysql->fetch_row($result);
	}
...
function get_matauang() {
	global $mysql;
	$sql = $mysql->query("SELECT matauang FROM catalogdata WHERE 1 LIMIT 1");
	list($matauang) = $mysql->fetch_row($sql);
...
function send_email_to_admin($trans_id) {
	...
	$sql = $mysql->query("SELECT transaction_id, receiver_id, tanggal_konfirmasi, bank_tujuan, bank_asal, atas_nama, no_rekening, nominal FROM sc_transaction WHERE transaction_id='$trans_id'");
...
// Ubah Status
	...
	$q = $mysql->query("SELECT value FROM contact WHERE name='umbalemail'");
	list($umbalemail) = $mysql->fetch_row($q);
	
	$q2 = $mysql->query("SELECT user_email FROM webmember WHERE user_id='".$_SESSION['member_uid']."'");
	list($email) = $mysql->fetch_row($q2);
	...
	if (!isset($_SESSION['member_uid'])) {
		$sql_r = $mysql->query("SELECT nama_pengirim, email, email_pengirim FROM sc_receiver WHERE receiver_id='$receiver_id'");
		// echo "SELECT nama_pengirim, email, email_pengirim FROM sc_receiver WHERE receiver_id='$receiver_id'<br>";
		list($sendername, $r_email, $r_email_pengirim) = $mysql->fetch_row($sql_r);
		if ($sendername != '') {
...
function send_email_to_admin_upload($trans_id) {
	global $smtpuser, $confirm_thanks_client, $cfg_app_url, $pakaivoucher, $pakaiadditional;
	global $mysql, $attachment;
	
	// $sql = $mysql->query("SELECT transaction_id, receiver_id, tanggal_konfirmasi, bank_tujuan, bank_asal, atas_nama, no_rekening, nominal FROM sc_transaction WHERE transaction_id='$trans_id'");
	$sql = "SELECT t.transaction_id, m.fullname, t.receiver_id, c.nama_lengkap, c.customer_id, tanggal_konfirmasi, bank_tujuan, bank_asal, 
			atas_nama, no_rekening, nominal 
			FROM sc_transaction t
			LEFT JOIN sc_customer c ON c.customer_id=t.customer_id
			LEFT JOIN webmember m ON m.user_id=c.web_user_id
	...
	if (!isset($_SESSION['member_uid'])) {
		$sql_r = $mysql->query("SELECT nama_pengirim, email, email_pengirim FROM sc_receiver WHERE receiver_id='{$row['receiver_id']}'");
		// echo "SELECT nama_pengirim, email, email_pengirim FROM sc_receiver WHERE receiver_id='$receiver_id'<br>";
		list($sendername, $r_email, $r_email_pengirim) = $mysql->fetch_row($sql_r);
		if ($sendername != '') {
			$email = $r_email_pengirim;
	...
	//send to admin
	fiestophpmailer($umbalemail, $subject, $data_admin, $smtpuser, $row['nama_lengkap'], $email, $data_admin, $attachment);
