...
if( count($pecah)==2){
	...
	$queryid = $mysql->query($sqlid);
	$row=$mysql->fetch_array($queryid);
	$idparent= $row['id'];
}
	...
	$queryidparent = $mysql->query($sqlidparent);
	
	$row=$mysql->fetch_array($queryidparent);
	$idparent = $row['id'];
	...
	for($i=2;$i<count($pecah);$i++){	
		...
		$queryidchild = $mysql->query($sqlidchild);
		$rowchild=$mysql->fetch_array($queryidchild);
		$idchild= $rowchild['id'];
