...
//transaction_id 	autoid 	tanggal_buka_transaksi 	tanggal_tutup_transaksi 	customer_id 	receiver_id 	metode_pembayaran 	ongkir 	status 	catatan 	no_pengiriman 	latestupdate 	customer_id 	web_user_id 	nama_lengkap 	email 	alamat 	alamat2 	kota 	provinsi 	id_country 	zip 	telepon 	cellphone1 	cellphone2 
	try {
		$query=$mysql->query("select * from sc_transaction t,sc_customer c,webmember w where 	c.customer_id=t.customer_id AND w.user_id=c.web_user_id AND t.transaction_id='$id'");
		...
		/* $pinquery=$mysql->query("select * from webmember where not bbpin='******'");
		
		$i=0;
		while($rowpin = $mysql->fetch_array($pinquery))
		{
...
function leastcatoption($cat_id="") {
    global $catmenu, $topcatnamecombo, $mysql;
...
function show_me_the_images($isadmin=FALSE) {
    global $action, $cfg_per_page, $cfg_max_cols, $urlfunc, $pakaicart, $mysql;
	...
	while (list($photo_id, $photo_cat_id, $photo_filename, $photo_title, $photo_published, $photo_ishot, $keterangansingkat, $harganormal, $diskon, $matauang, $nilai_disc, $harga_disc,$isbest,$ispromo,$isnew,$issold,$is_filter,$brand_id) = $mysql->fetch_row($photo)) {
		
		// $sql = "SELECT nama FROM catalogfilterdata WHERE FIND_IN_SET(id, '$is_filter')";
		// $result = $mysql->query($sql);
		
		// $gridname = '';
		// while(list($grid_name) = $mysql->fetch_row($result)) {
        ...
        if ($action == 'brand') {
            ...
            $resultmerekname = $mysql->query($sqlmerekname);
            list($merekname) = $mysql->fetch_row($resultmerekname);
            $titleurl["merek"] = $merekname;
            $mysql->free_result($resultmerekname);
        }
        ...
        if ($isadmin) {
			...
			$catcontent .= '<a class="btn btn-default more" href="' . $urlfunc->makePretty("?p=catalog&action=detail&pid=$photo_id&$actiondependent", $titleurl) . '">View More</a></div>	<!-- /.product-col -->';
...
function show_me_the_list() {
    ...
    while (list($photo_id, $photo_cat_id, $photo_filename, $photo_title, $photo_published, $photo_ishot, $keterangan, $harganormal, $diskon, $matauang, $nilai_disc, $harga_disc) = $mysql->fetch_row($photo)) {
        $titleurl = array();
...
function show_catalog_grid()
{
    ...
    $sql .= " ORDER BY $sort ";
    $result = $mysql->query($sql);
	$jum_product=$mysql->num_rows($result);
    if ($jum_product > 0) {
...
function save_attribut_tambahan($id)
{
	...
	$q=$mysql->query("UPDATE catalogdata set attribut_tambahan='$serialize' WHERE id=$id");
...
function attribut_tambahan($pid="")
{
	...
	if($pid!='')
	{
		$r_serial=$mysql->query("SELECT attribut_tambahan FROM catalogdata WHERE id=$pid");
		list($serialdata)=$mysql->fetch_row($r_serial);
	...
	if($res and $mysql->num_rows($res)>0)
	{
		while($d=$mysql->fetch_assoc($res))
		{
			...
			$d['nama'] = strtolower($d['nama']);
				...
				{
					$data=array();
					$resd=$mysql->query("SELECT id,id_atm,value FROM catalog_atd WHERE id_atm=".$d['id']." ORDER BY id");
					if($resd and $mysql->num_rows($resd)>0)
					{
						
						while($e=$mysql->fetch_assoc($resd))
						{
							$data[$e['id']]=$e['value'];
...
function get_product_attr($catalog_id, $color) {
	global $mysql;
	
	$data = array();
	
	$sql = "SELECT id, size, color, price FROM catalogattribut WHERE catalog_id='$catalog_id' and color='$color' ORDER BY urutan";
		...
		if ($mysql->num_rows($result) > 0) {
			...
			$sql_catalog = "SELECT harganormal, if(LOCATE('%',diskon)>0,(TRIM(REPLACE(diskon,'%',''))/100*harganormal),diskon) 
					...
					harganormal-(if(LOCATE('%',diskon)>0,(TRIM(REPLACE(diskon,'%',''))/100*harganormal),
						diskon)) as hargadiskon FROM catalogdata WHERE id='$catalog_id'";
			$result_catalog = $mysqli->query($sql_catalog);
			list($harganormal, $nilaidiskon, $hargadiskon) = $mysql->fetch_row($result_catalog);			
