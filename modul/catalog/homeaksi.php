	...
	if ($jenis == 'featured' || $jenis == 'category') {
		switch ($jenis) {
			...
			case 'category':
				...
				$limit = "";
				$q = $mysql->query("SELECT value FROM config WHERE name='cfg_per_page_home' and modul='catalog'");
				if($q AND $mysql->num_rows($q)>0) {
					list($max_list)=$mysql->fetch_row($q);
		...
		if ($result and $mysql->num_rows($result) > 0) {
			...
			if ($showtype == 0) {
				...
				while (list($photo_id, $photo_cat_id, $photo_filename, $photo_title, $photo_published, $photo_ishot, $keterangan, $harganormal, $diskon, $matauang, $nilai_disc, $harga_disc,$isbest,$ispromo,$isnew,$issold, $brand_id) = $mysql->fetch_row($result)) {
			...
			if ($showtype == 1) {
				...
				while (list($photo_id, $photo_cat_id, $photo_filename, $photo_title, $photo_published, $photo_ishot, $keterangan, $harganormal, $diskon, $matauang, $nilai_disc, $harga_disc, $catname) = $mysql->fetch_row($result)) {
					$titleurl = array();
					$titleurl["pid"] = $photo_title;
...
if ($jenis == 'featuredmarquee' || $jenis == 'categorymarquee') {
    ...
    if ($mysql->num_rows($result) > 0) {
		...
		// $home .= "<div class=\"item\">";
        // $home .= "<ul>";
        while (list($photo_id, $photo_cat_id, $photo_filename, $photo_title, $photo_published, $photo_ishot, $keterangansingkat, $harganormal, $diskon, $matauang, $nilai_disc, $harga_disc,$isbest,$ispromo,$isnew,$issold,$brand_id) = $mysql->fetch_row($result)) {
			$brand_name = get_brand_name($brand_id);
			$cat_name = get_cat_name($photo_cat_id);
