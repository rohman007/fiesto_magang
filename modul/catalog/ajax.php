...
$s=$mysql->query("INSERT INTO catalogcat(parent,nama,urutan) values($cat_id,'$text_cat',0)");
/////////
echo "<select name=\"cat_id\" onchange=\"tambah_cat(this.value)\">\n";
		$cats = new categories();
		$mycats = array();
...
$s=$mysql->query("INSERT INTO catalogmerek(nama) values('$brand')");
echo '<select name="merek" onchange="tambah_brand(this.value)">';
	$result = $mysql->query("SELECT id,nama FROM catalogmerek ORDER BY nama ");
	echo "<option value=\"\" >"._NOBRAND."</option>\r\n";
	while(list($idmerek,$namamerek) = $mysql->fetch_row($result)) {
...
$list=$_POST['list'];
	...
	foreach($list as $cat_id =>$parent)
	{
		...
		{
		$s=$mysql->query("UPDATE catalogcat SET parent='$parent',urutan=$urutan WHERE id='$cat_id'");
		}
		else
		{
		$s=$mysql->query("UPDATE catalogcat SET parent='0',urutan=$urutan WHERE id='$cat_id'");
		}
	$urutan++;	
	}
...
if ($action == 'param_color') {
	$color = fiestolaundry($_GET['color'], 30);
	$pid = fiestolaundry($_GET['pid'], 11);
	
	$html = '';
	$sql = "SELECT id, size, color, price FROM catalogattribut WHERE catalog_id='$pid' AND size='$color' ORDER BY urutan";
	if ($result = $mysql->query($sql)) {
		if ($mysql->num_rows($result) > 0) {
