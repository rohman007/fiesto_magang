...
//        $judul = checkrequired($judul, _TITLE);
		...
		{
            // list($idpage) = mysql_fetch_row(mysql_query("SELECT max(id) FROM page"));
            // $idpage = $idpage + 1;
            ...
            // if ($result) {
                ...
                // // list($idmenu) = mysql_fetch_row(mysql_query("SELECT max(id) FROM page"));
...
if ($action == "modify" || $action == "" ) {
    ...
    if (mysql_num_rows($result) == "0") {
        ...
        // list($id, $judul, $template) = mysql_fetch_row($result);
        // $publishstatus = ($publish == 1) ? 'checked' : '';
        //$catselect = adminselectcategories('linkcat',$cat_id);
    	//get isi from menu type
		// $q=mysql_query("SELECT id,parent FROM menu WHERE  type='5' AND isi='$pid'");
		// if($q and mysql_num_rows($q)>0)
		// {
		// list($idmenu,$subparent)=mysql_fetch_row($q);
		// }
		...
		<form class="form-horizontal" method="POST" action="' . $thisfile . '" enctype="multipart/form-data">
			...
			<input type="hidden" name="pid" value="' . $pid . '">';
			 while(list($id, $judul, $template) = mysql_fetch_row($result)) {
				$admincontent .= '
					 <div class="control-group">
...
// if ($action == "") {
    ...
    // if (mysql_num_rows($result) > 0) {
        ...
        // while (list($id, $judul, $rowtemplate) = mysql_fetch_row($result)) {
            // $admincontent .= "<tr>\n";
