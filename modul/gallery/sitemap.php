<?php
    ...
    for ($i = 0; $i < count($cats->cats); $i++) {
		...
		$url = $urlfunc->makePretty("?p=gallery&action=images&cat_id=" . $cats->cats[$i]['id'], $titleurl);
	...
	$sql = "SELECT id, cat_id, title FROM gallerydata WHERE publish='1' ORDER BY cat_id, id";
    $result = $mysql->query($sql);
    while (list($id,$cat_id, $title) = $mysql->fetch_array($result)) {
        $titleurl["pid"] = $title;	//untuk menampilkan nama produk di prettyurl
		$url = $urlfunc->makePretty("?p=gallery&action=viewimages&pid=$id&cat_id=$cat_id", $titleurl);
		$xml_standalone .= "
				<url>
					<loc>".xmlurl($url)."</loc>
					<lastmod>$tanggal</lastmod>
