 ...
 */
    ...
    if ($mysql->num_rows($result) > 0) {
        ...
        $admincontent .= "<th align=\"center\">" . _EDIT . "</th>";
        //$admincontent .= "<th>"._DEL."</th>";
        $admincontent .= "</tr>\n";

        while ($data_member = $mysql->fetch_array($result)) {
	...
	if (!$mysql->query($sql)) {
		...
		if (count($daftar_id) > 0) {
			...
			foreach($daftar_id as $user_id) {
				$sql = "SELECT user_email FROM webmember WHERE user_id=$user_id";
				$result = $mysql->query($sql);
				list($user_email, $type_company) = $mysql->fetch_row($result);
				
				include("$cfg_app_path/modul/$namamodul/lang/$lang/mailactv.php");
				
				fiestophpmailer($user_email,$subject,$message,$smtpuser,$smtpuser,$smtpuser);
			}
		}
...
if ($action == 'add') {
    if (isset($_POST['submit'])) {
        ...
        if ($notificationbuilder != "") {
            ...
            $sql = "INSERT INTO $tabelwebmember SET username='$postuser', 
				user_password='$cookedpass', fullname='$postnama',
				company='$postperusahaan', address1='$postalamat1', activity='1', 
            ...
            if ($result) {
                ...
                if ($is_public_activation) {
                    ...
                    $resultcontact = $mysql->query($sqlmailcontact);
                    list($emailadmin) = $mysql->fetch_row($resultcontact);
	...
	<form class=\"form-horizontal\" method="POST" action="' . $thisfile . '" enctype="multipart/form-data">
			...
			<!--<div class="control-group">
					...
					<div class="controls">
							<select name="level">get_dropdown_level()</select>
					</div>
			</div>-->		
            <div class="control-group">
...
if ($action == "modify") {
    if (isset($_POST['submit'])) {
        ...
        $sql = "SELECT fullname, company, address1, address2, city, zip, telephone, 
				cellphone1, cellphone2, fax, user_email, level
			FROM $tabelwebmember WHERE user_id='$pid'";
        $result = $mysql->query($sql);
        list($postnama, $postperusahaan, $postalamat1, $postalamat2, $postkota, $postkodepos,
                $posttelepon, $postponsel1, $postponsel2, $postfax, $postemail, $level) = $mysql->fetch_row($result);
        $activeProfile = "active";
        $activePassword = "";
    }
                    ...
                    <div class="tab-pane ' . $activeProfile . '" id="profile">
                        <form class=\"form-horizontal\" method="POST" action="' . $thisfile . '" enctype="multipart/form-data">
                            ...
                            <!--<div class="control-group">
                                    <label class="control-label">' . _LEVEL . '*</label>
                                    <div class="controls">
                                            <select name="level">get_dropdown_level($level)</select>
                                    </div>
                            </div>-->
                            <div class="control-group">
                                    <label class="control-label">' . _COMPANY . '</label>
